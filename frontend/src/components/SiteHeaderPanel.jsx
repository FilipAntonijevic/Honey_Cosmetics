...
export default function SiteHeaderPanel({
  ...
  const bannerSegments = (notificationBanner?.text || '')
    .split('•')
    .map((s) => s.trim())
    .filter(Boolean)

  const topStrip = notificationBanner?.enabled && bannerSegments.length > 0 ? (
    <div className="top-strip">
      <div className="ticker-track">
        {[...Array(4)].map((_, i) => (
          <span key={i} className="ticker-item">
            {bannerSegments.map((seg, j) => (
              <span key={`${i}-${j}`}>
                {seg}
                {j < bannerSegments.length - 1 && <span className="ticker-sep">•</span>}
              </span>
            ))}
  ...
  const wishlistControl = (
    <Link to="/wishlist" className="icon-btn" title="Wishlist" tabIndex={tabOff}>
      ...
      {wishlistReady && wishlist.length > 0 && <span className="icon-badge">{wishlist.length}</span>}
    </Link>
  )
      ...
      {userMenuOpen && attachRefs && (
        <div className="user-menu">
          ...
          <Link
            ...
            onClick={(e) => { e.preventDefault(); onUserMenuNavigate('/my-orders') }}
          >
          ...
          <Link
            to="/profile"
            className="user-menu-item"
            onClick={(e) => { e.preventDefault(); onUserMenuNavigate('/profile') }}
          >
        ...
        <div className="phone-menu" role="menu">
          {phoneHref ? (
            <a
              ...
              role="menuitem"
              onClick={(e) => { e.preventDefault(); onPhoneMenuAction(phoneHref) }}
            >
          ...
          {whatsAppHref ? (
            <a
              ...
              className="phone-menu-item"
              role="menuitem"
              onClick={(e) => { e.preventDefault(); onPhoneMenuAction(whatsAppHref, { newTab: true }) }}
            >
          ...
          {viberHref ? (
            <a
              ...
              className="phone-menu-item"
              role="menuitem"
              onClick={(e) => { e.preventDefault(); onPhoneMenuAction(viberHref) }}
            >
      ...
      {categoriesMenuOpen && attachRefs && (
        <div className="categories-dropdown" role="menu">
          ...
          <Link
            ...
            onClick={(e) => {
              e.preventDefault()
              onCategoriesMenuNavigate('/shop?bestsellers=1')
            }}
          >
            Bestsellers
          </Link>
